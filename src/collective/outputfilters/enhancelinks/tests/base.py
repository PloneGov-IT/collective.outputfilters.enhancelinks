...
class BaseTest(unittest.TestCase):
    ...
    def extract_mimetype(self, file_item):
        content_type = api.env.plone_version() < '5.0' and \
            file_item.getContentType() or file_item.contentType
    ...
    def get_attachment(self, filename, type):
        ...
        if type == 'image':
            return NamedImage(data=open(file_path), filename=filename)
        return NamedFile(data=open(file_path), filename=filename)
